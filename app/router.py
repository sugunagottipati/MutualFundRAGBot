...
class IntentRouter:
    ...
    def __init__(self):
        ...
        self.factual_patterns = [
            ...
            r"show.*sector(?:s)?\s+(?:allocation|breakdown)",
            ...
            r"10.?year.*return",
            r"top.*holdings?",
            r"(?:what|who).*fund\s+manager",
            r"who\s+manages",
            r"fund\s+manager.*(?:since|tenure|start)",
            r"since when.*fund\s+manager",
            r"when.*was.*fund.*launched",
            r"(?:when|what).*inception",
            r"what.*is.*the.*fund.*size",
            r"(?:what|how much).*\b(?:aum|assets under management)\b",
            r"(?:what|which).*investment objective",
            r"what.*does.*fund.*aim",
            r"(?:which|what).*fund house",
            r"(?:what|which).*\b(?:amc|asset management company)\b",
            r"what.*(?:fund )?category",
            r"which.*category.*(?:belong|fall|fund)",
            r"(?:what|is this).*plan type",
            r"(?:direct|regular)\s+plan",
            r"growth\s+plan",
            r"(?:what|which).*tax(?:ation| implications?)?",
            r"how.*(?:fund|scheme).*taxed",
            r"capital gains tax",
            r"what.*stamp duty",
            r"stamp duty.*(?:charge|rate|apply)",
            r"category average",
            r"(?:what|which).*rank.*(?:category|fund)",
            r"ranking.*(?:category|fund)",
        ...
        self.comparative_patterns = [
            r"compare",
            r"which.*is.*better",
            r"rank\s+(?:these|the|all|mutual|hdfc)\s+funds?",
            r"best.*performing",
            r"top\s+funds?",
            r"outperform",
            r"better.*than",
